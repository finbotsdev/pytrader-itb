...
class Trader:
  def __init__(self, APCA_API_KEY_ID, APCA_API_SECRET_KEY, _L, account):
    self._L = _L
    self.thName = threading.currentThread().getName()

    try:
      self.APCA_API_KEY_ID = APCA_API_KEY_ID
      self.APCA_API_SECRET_KEY = APCA_API_SECRET_KEY
      self.APCA_API_ENDPOINT = "https://paper-api.alpaca.markets"
      self.alpaca = tradeapi.REST(self.APCA_API_KEY_ID, self.APCA_API_SECRET_KEY, self.APCA_API_ENDPOINT, api_version='v2') # or use ENV Vars

    except Exception as e:
      self._L.info('ERROR_IN: error when initializing: ' + str(e))
      block_thread(self._L, e, self.thName)

    self.operEquity = config.operEquity
    self.pctMargin = config.limitOrderMargin/100
  ...
  def set_stoploss(self, stopLoss, direction):
    ...
    try:
      if direction is 'buy':
        self.stopLoss = float(stopLoss - stopLoss*config.stopLossMargin)
  ...
  def set_takeprofit(self, entryPrice, stopLoss):
    ...
    # depending on the gainRatio defined at the config file

    diff = entryPrice - stopLoss
    try:
      self.takeProfit = round(entryPrice + diff*config.gainRatio, 2)
  ...
  def load_historical_data(self, stock, interval='1Min', limit=100):
    ...
    while True:
      try: # fetch the data
        ...
        self._L.info(e)
        time.sleep(config.sleepTimes['LH'])
      ...
      try: # check if the data is updated
        ...
        if diff <= timedeltaItv:
          ...
          if config.maxAttempts['LHD1'] >= attempt >= config.maxAttempts['LHD2']:
  ...
  def submitOrder(self, orderDict):
    ...
    attempt = 0
    while attempt < config.maxAttempts['SO']:
      try:
        if type is 'limit':
        ...
        self._L.info(str(e))
        time.sleep(config.sleepTimes['SO'])
        attempt += 1
  ...
  def cancelOrder(self, orderId):
    ...
    attempt = 0
    while attempt < config.maxAttempts['CO']:
      try:
  ...
  def check_position(self, stock, maxAttempts=False):
    ...
    if not maxAttempts:
      maxAttempts = config.maxAttempts['CP']
    ...
    while attempt < maxAttempts:
      try:
        ...
        time.sleep(config.sleepTimes['CP'])
        attempt += 1
  ...
  def check_general_trend(self, stock):
    ...
    try:
      while True:
        self.load_historical_data(stock, interval=config.fetchItval['big'])
        ...
        # check the selling trend
        ...
        elif timeout >= config.timeouts['GT']:
          ...
          timeout += config.sleepTimes['GT']
          time.sleep(config.sleepTimes['GT'])
  ...
  def check_instant_trend(self, stock, loadHist=False, wait=True):
    ...
    try:
      while True:
        if loadHist:
            self.load_historical_data(stock, interval=config.fetchItval['little'])
        ...
        # look for a selling trend
          ...
          if wait:
            self.timeout += config.sleepTimes['IT']
            time.sleep(config.sleepTimes['IT'])
  ...
  def check_rsi(self, stock, loadHist=False):
    ...
    while True:
      ...
      if (stock.direction == 'buy') and ((rsi>50) and (rsi<80)):
        ...
        self.timeout += config.sleepTimes['RS']
        time.sleep(config.sleepTimes['RS'])
        return False

  def check_stochastic(self, stock, direction, loadHist=False):
    ...
    try:
      while True:
            ...
            if (
                    (direction == 'buy') and
                    (stoch_k > stoch_d) and
                    ((stoch_k <= config.limStoch['maxBuy']) and (stoch_d <= config.limStoch['maxBuy']))
                ):
            ...
            # look for a selling condition
                    ...
                    (direction == 'sell') and
                    (stoch_k < stoch_d) and
                    ((stoch_d >= config.limStoch['minSell']) and (stoch_k >= config.limStoch['minSell']))
                ):
                ...
                self.timeout += config.sleepTimes['ST']
                time.sleep(config.sleepTimes['ST'])
                return False
  ...
  def enter_position_mode(self, stock, desiredPrice, sharesQty):
    ...
    while True:
      ...
      if (stochTurn >= config.sleepTimes['GS']) or (timeout == 0):
        # check the stochastic crossing
        stochTurn = 0
        self.load_historical_data(stock, interval=config.fetchItval['little'])
        stochCrossed = self.check_stochastic(stock, direction=reverseDirection)
      ...
      # if stochastics crossed otherwise
        ...
        time.sleep(config.sleepTimes['PF'])
        timeout += config.sleepTimes['PF']
        stochTurn += config.sleepTimes['PF']
  ...
  def run(self, stock):
    ...
    # check if the position exists beforehand
    if self.check_position(stock, maxAttempts=2):
    ...
    # check the trend
    if not self.check_general_trend(stock):
      return stock.name, True

    # can it be traded?
    if not self.is_tradable(stock.name, stock.direction):
      return stock.name, True
    ...
    while True:
      ...
      # 2. INSTANT TREND
      if not self.check_instant_trend(stock):
        continue # restart the loop

      # 3. RSI
      if not self.check_rsi(stock):
        continue # restart the loop

      # 4. STOCHASTIC
      if not self.check_stochastic(stock, direction=stock.direction):
        continue # restart the loop
